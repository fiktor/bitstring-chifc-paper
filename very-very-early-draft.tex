  ...
  \begin{enumerate}
    ...
    \item If $\rho$ and $\sigma$ are diagonal density matrices with diagonal
      ...
      \begin{equation}
        F(\rho, \sigma) = F(p, q).
      \end{equation}
    \item $F$ is symmetric with respect to changing the order of its arguments.
      In particular, for 2 density matrices $\rho$ and $\sigma$ we have
...
\begin{proof}
  ...
  Let's prove part 3.
  Note that the formulas \eqref{eq:def.F.pure} and \eqref{eq:def.Fc} are
  symmetric with respect to the order of arguments, so the only nontrivial
  case is for density matrices $\rho$ and $\sigma$ and formula
...
\begin{center}
  ...
  \begin{tikzpicture}[scale=\tikzscale]
    ...
    \draw (0, 0) node {\includegraphics[width=\columnwidth]{ladder-ed-chifc.png}};
