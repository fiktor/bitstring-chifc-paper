...
\usepackage{graphics,epstopdf,amsthm,amsmath,amssymb,mathptmx,colortbl,color,bm,framed,mathrsfs}
...
\begin{itemize}
%  \item Formula \eqref{eq:def.F.mixed} is consistent with \eqref{eq:def.F.pure} for pure states and with \eqref{eq:def.Fc} for probability distributions.
...
%%DL: this is well known, so need not be presented as a generalization
%Before moving to fidelity susceptibility, we 
%%want to 
%describe a generalization of the fidelity susceptibility covered in \cite{zanardi2007information}.
%Formulas 
...
%the metric \eqref{eq:def.gmetric} 
$g$ in the
...
\begin{theorem}
  ...
  \begin{enumerate}
    \item Suppose $\ket{\psi(\lambda)}$ is a state defined in the
      ...
      \end{multline}
%      where $\partial_\mu \psi(\lambda_0)$ is a compact notation for the derivative $\left.\frac{\partial \psi(\lambda)}{\partial \lambda^\mu} \right|_{\lambda=\lambda_0}$.
...
%fact that the Hilbert space elements representing the states have unit norm. 
Since we do
...
\DL{Best to generate the complete figure as a pdf and not use tikz; I've sometimes had trouble with publishers when using tikz.}
\begin{center}
...
\begin{proof}
  ...
  \begin{equation}
    ...
    P(z|s\pm \delta s / 2) / Q(z) =
      \frac{e^{\pm M(s, \delta s, z) \delta \lambda/2}}{
        \cosh(M(s, \delta s, z) \delta \lambda/2)
      }.
  \end{equation}
